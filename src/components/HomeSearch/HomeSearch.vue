<template>
    <div class="container mt-5">
        <h1>Home search made easy</h1>
        <p class="my-3">Discover how we can help you find your dream home</p>
        <div class="row">
            <div class="col-md-6">
                <!-- <div class="card mb-3">
                    <div class="img-wrapper">
                        <img src="../../assets/avatar/av1.jpg" class="card-img-top inner-img" height="375px" alt="...">
                    </div>
                    <div class="card-img-overlay">
                        <span class="badge text-bg-secondary card-title">SUPERAGENT</span>
                    </div>
                    <div class="card-body bg-body-tertiary">
                        <h5 class="card-title">Find your SuperAgent</h5>
                        <p class="card-text">The most responsive agent with up-to-date and improved accuracy on the
                            properties you are searching for.</p>
                    </div>
                </div> -->
                <div class="card mb-3">
                    <div class="img-wrapper">
                        <img src="../../assets/avatar/av1.jpg" class="card-img-top inner-img" height="375px" alt="...">
                    </div>
                    <div class="card-body bg-body-tertiary">
                        <span class="badge text-bg-secondary card-title"><img src="../../assets/tropyBadgeIcon/pngegg.png" class="" style="height: 15px;" alt=""> SUPERAGENT</span>
                        <h5 class="card-title">Find your SuperAgent</h5>
                        <p class="card-text">The most responsive agent with up-to-date and improved accuracy on the
                            properties you are searching for.</p>
                    </div>
                </div>
            </div>
            <div class="col-md-6">
                <!-- <div class="card mb-3">
                    <div class="img-wrapper">
                        <img src="../../assets/avatar/av2.jpg" class="card-img-top inner-img" height="400px" alt="...">
                    </div>
                    <div class="card-img-overlay">
                        <span class="badge text-bg-secondary card-title">VERIFIED</span>
                    </div>
                    <div class="card-body bg-body-tertiary">
                        <h5 class="card-title">verified Listings</h5>
                        <p class="card-text">Explore property listing that have passed our verification process.</p>
                    </div>
                </div> -->
                <div class="card mb-3">
                    <div class="img-wrapper">
                        <img src="../../assets/avatar/av2.jpg" class="card-img-top inner-img" height="400px" alt="...">
                    </div>
                    <div class="card-body bg-body-tertiary">
                        <span class="badge text-bg-success card-title"><img src="../../assets/tropyBadgeIcon/verified.png" style="height: 15px;" alt="" /> VERIFIED</span>
                        <h5 class="card-title"> verified Listings</h5>
                        <p class="card-text">Explore property listing that have passed our verification process.</p>
                    </div>
                </div>
            </div>
        </div>
    </div>
</template>
<script>

</script>
<style>
.inner-img {
    transition: 0.3s;
}

.inner-img:hover {
    transform: scale(1.1);
}

.img-wrapper {
    width: 100%;
    height: 100%;
    overflow: hidden;
}
</style>